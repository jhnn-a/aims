...
import {
  getDeviceHistoryByTag,
  getDeviceHistoryById,
  createSampleDeviceHistory,
  ...
  const fetchDeviceHistory = async () => {
    try {
      ...
      if (err.message && err.message.includes("index")) {
        setError(
          "Database index not configured. Please contact your administrator."
        );
      } else {
  ...
  const getActionColor = (action) => {
    if (isDarkMode) {
      ...
      switch (action) {
        ...
        case "unassigned":
          return "#fbbf24"; // Lighter yellow/amber
        case "returned":
    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div style={styles.header}>
          <h2 style={styles.title}>Device History - {deviceTag || deviceId}</h2>
          <button
