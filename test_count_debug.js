...
console.log(
  "🎯 Expected from UnitSpecs: 1 Critical + 1 Needs Maintenance = 2 total devices"
);
console.log(
  "❌ Dashboard showing: 3 Critical + 0 Needs Maintenance + 0 Healthy = 3 total devices"
);
...
console.log(
  "3. Dashboard might be including devices without maintenance tracking"
);
console.log(
  "4. Fallback logic is too aggressive (defaulting everything to Critical)"
);

console.log("\n🔧 Solution Applied:");
console.log(
  "- Filter devices to only count those with Tag + deviceType (actual UnitSpecs devices)"
);
console.log(
  "- Default devices without maintenance data to 'Healthy' instead of age-based logic"
);
console.log(
  "- Only calculate for devices that would actually appear in UnitSpecs"
);
...
testDevices.forEach((device, index) => {
  ...
  const completedTasks = checklistKeys.filter(
    (key) => device.maintenanceChecklist[key].completed
  );
  const completionRate =
    checklistKeys.length > 0
      ? (completedTasks.length / checklistKeys.length) * 100
      : 0;
  ...
  console.log(
    `   - Maintenance tasks: ${completedTasks.length}/${
      checklistKeys.length
    } completed (${completionRate.toFixed(1)}%)`
  );
...
console.log(
  "📋 Check browser console for actual device data being processed by Dashboard."
);
...
console.log(
  "4. Check that maintenance status calculation matches UnitSpecs badges"
);
